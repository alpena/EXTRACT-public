...
script_log = '';
log_extract_gpu_state(config, 'run_extract:start', ...
    sprintf('movie=%dx%dx%d', fov_size(1), fov_size(2), n));
...
if isempty(config.S_init)
    ...
    script_log = [script_log, str];
    dispfun(str, config.verbose ==2);
    log_extract_gpu_state(config, 'cw_extract:start', ...
        sprintf('movie=%dx%dx%d', fov_size(1), fov_size(2), n));
    [S, T, init_summary] = cw_extract(M, config);
    log_extract_gpu_state(config, 'cw_extract:end', ...
        sprintf('cells=%d', size(T, 1)));
...
for iter = 1:config.max_iter
    ...
    log_extract_gpu_state(config, sprintf('solve_T:iter%d:start', iter), ...
        sprintf('cells=%d frames=%d', size(T, 1), size(Mt, 1)));
    ...
    log_extract_gpu_state(config, sprintf('solve_T:iter%d:retry', iter), ...
        sprintf('cells=%d frames=%d', size(T, 1), size(Mt, 1)));
    [T, loss, np_x, np_y, np_time] = solve_T(T, S, Mt, fov_size, avg_radius, lambda, ...
            kappa, config.max_iter_T, config.TOL_sub, ...
    ...
    log_extract_gpu_state(config, sprintf('solve_T:iter%d:end', iter), ...
        sprintf('cells=%d np=%dx%dx%d', size(T, 1), np_x, np_y, np_time));
    ...
    if config.adaptive_kappa_filter
        ...
        log_extract_gpu_state(config, sprintf('solve_S:iter%d:start', iter), ...
            sprintf('cells=%d frames=%d', size(T, 1), size(Mt, 1)));
        ...
        log_extract_gpu_state(config, sprintf('solve_S:iter%d:retry', iter), ...
            sprintf('cells=%d frames=%d', size(T, 1), size(Mt, 1)));
        [S, loss, np_x, np_y, T_corr_in, T_corr_out, S_surround] = solve_S(...
            S, T, Mt, mask, fov_size, avg_radius, ...
                lambda, kappa, config.max_iter_S, config.TOL_sub, ...
    ...
    log_extract_gpu_state(config, sprintf('solve_S:iter%d:end', iter), ...
        sprintf('cells=%d np=%dx%d', size(S, 2), np_x, np_y));
