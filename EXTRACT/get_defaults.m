function config = get_defaults(config)
    ...
    if ~isfield(config, 'cellfind_batch_size'), config.cellfind_batch_size = 1; end
    if ~isfield(config, 'cellfind_batch_distance_factor'), config.cellfind_batch_distance_factor = 2.5; end
    if ~isfield(config, 'cellfind_batch_trace_corr_thresh'), config.cellfind_batch_trace_corr_thresh = 0.3; end
    if ~isfield(config, 'cellfind_batch_max_candidates'), config.cellfind_batch_max_candidates = 256; end
    if ~isfield(config, 'cellfind_batch_fit_mode'), config.cellfind_batch_fit_mode = 'single'; end
    if ~isfield(config, 'cellfind_batch_refine_iter'), config.cellfind_batch_refine_iter = 0; end
